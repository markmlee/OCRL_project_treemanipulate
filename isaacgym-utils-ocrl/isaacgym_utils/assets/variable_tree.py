...
class GymVarTree(GymURDFAsset):
    ...
    def __init__(self, cfg, urdf_path, name_dict, *args, actuation_mode='joints'):
        ...
        self.joint_names = name_dict["joints"]
        self.link_names = name_dict["links"]
        self.num_joints = len(self.joint_names)
        self.num_links = len(self.link_names)
        self.init_joints = np.zeros(self.num_joints)
    ...
    def set_actuation_mode(self, mode, env_idx, name):
        ...
        if self._actuation_mode == 'attractors':
            self.set_dof_props(env_idx, name, {
                'driveMode': [gymapi.DOF_MODE_NONE] * self.num_joints
            })
            ...
            self.set_dof_props(env_idx, name, {
                'driveMode': [gymapi.DOF_MODE_POS] * self.num_joints
            })
        elif self._actuation_mode == 'torques':
            self.set_dof_props(env_idx, name, {
                'driveMode': [gymapi.DOF_MODE_EFFORT] * self.num_joints
            })
        else:
